...
If you want to build InternationalConsumptionChain-Qt, make sure that the required packages for Qt development
...
Once these are installed, they will be found by configure and a InternationalConsumptionChain-Qt executable will be
built by default.
...
```bash
InternationalConsumptionChain_ROOT=$(pwd)

# Pick some path to install BDB to, here we create a directory within the ion directory
BDB_PREFIX="${InternationalConsumptionChain_ROOT}/db4"
mkdir -p $BDB_PREFIX
...
cd $InternationalConsumptionChain_ROOT
./autogen.sh
...
2) The release is built with GCC and then "strip InternationalConsumptionChaind" to strip the debug
...
With UPNP:
    ...
    strip InternationalConsumptionChaind
...
To Build InternationalConsumptionChain-Qt
--------
