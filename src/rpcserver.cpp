...
Value stop(const Array& params, bool fHelp)
{
    ...
    if (fHelp || params.size() > 1)
        throw runtime_error(
            "stop\n"
            "Stop InternationalConsumptionChain server.");
    // Shutdown will take long enough that the response should get back
    StartShutdown();
    return "InternationalConsumptionChain server stopping";
}
...
void StartRPCThreads()
{
         ...
         (mapArgs["-rpcuser"] == mapArgs["-rpcpassword"])) && Params().RequireRPCPassword())
    {
        ...
        string strWhatAmI = "To use InternationalConsumptionChaind";
...
std::string HelpExampleCli(string methodname, string args){
    return "> InternationalConsumptionChaind " + methodname + " " + args + "\n";
}
