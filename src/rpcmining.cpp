...
Value checkkernel(const Array& params, bool fHelp)
{
    ...
    if (vNodes.empty())
        throw JSONRPCError(-9, "InternationalConsumptionChain is not connected!");

    if (IsInitialBlockDownload())
        throw JSONRPCError(-10, "InternationalConsumptionChain is downloading blocks...");
...
Value getwork(const Array& params, bool fHelp)
{
    ...
    if (vNodes.empty())
        throw JSONRPCError(RPC_CLIENT_NOT_CONNECTED, "InternationalConsumptionChain is not connected!");

    if (IsInitialBlockDownload())
        throw JSONRPCError(RPC_CLIENT_IN_INITIAL_DOWNLOAD, "InternationalConsumptionChain is downloading blocks...");
